...
from app.modules.ingesta.etl.validator import validate_record_xlsx
...
def test_valid_record_passes():
    assert validate_record_xlsx(_record()) is None


def test_rejects_cell_id_zero():
    assert validate_record_xlsx(_record(**{"Cell ID/ECI": 0})) == MOTIVO_CELL_ID_CERO


def test_rejects_rsrp_sentinel():
    assert validate_record_xlsx(_record(RSRP=99)) == MOTIVO_RSRP_CENTINELA


def test_rejects_gps_sin_fix():
    assert validate_record_xlsx(_record(Latitud=0, Longitud=0)) == MOTIVO_GPS_SIN_FIX
...
def test_cell_id_zero_takes_precedence_over_rsrp_sentinel():
    ...
    assert validate_record_xlsx(record) == MOTIVO_CELL_ID_CERO
